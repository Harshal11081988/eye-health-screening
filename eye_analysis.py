...
def _find_cascade_path(filename):
    ...
    if not hasattr(cv2, "CascadeClassifier"):
        raise RuntimeError(
            "cv2.CascadeClassifier is not available in this OpenCV build. "
            "OpenCV 5.0+ moved Haar cascades to opencv-contrib -- make sure "
            "requirements.txt pins opencv-python-headless<5.0 (see README)."
        )
    candidates = []
    try:
