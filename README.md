...
## Known dependency issue (found in production, now fixed)

**OpenCV 5.0 removed `cv2.CascadeClassifier` from the main package**,
moving Haar cascades to `opencv-contrib`. This project was built and
tested against OpenCV 4.13, and `requirements.txt` originally allowed
any version `>=4.9` with no upper bound — when OpenCV 5.0 released and
Streamlit Cloud rebuilt the app, it resolved to `opencv-python-headless
5.0.0.93`, and the app broke with `AttributeError: module 'cv2' has no
attribute 'CascadeClassifier'`.

Fixed by pinning `opencv-python-headless>=4.9,<5.0` in
`requirements.txt`, plus a defensive check in `eye_analysis.py` that
raises a clear, actionable error (instead of a cryptic AttributeError)
if this ever recurs. The rPPG Stress Estimator project had the same
latent issue and was fixed the same way.
